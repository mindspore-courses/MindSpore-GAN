...
def g_forward(z):
    ...
    loss_G = -ops.mean(discriminator(gen_imgs))
    return loss_G, gen_imgs


# 判别器正向传播
def d_forward(real_imgs, z):
    fake_imgs = ops.stop_gradient(generator(z))
    loss_D = -ops.mean(discriminator(real_imgs)) + ops.mean(discriminator(fake_imgs))
